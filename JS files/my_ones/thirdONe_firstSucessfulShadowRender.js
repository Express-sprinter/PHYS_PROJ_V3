...
Promise.all([
  new Promise(res => loader.load('/models/sphere.obj', obj => res({ obj, scale: 0.3, pos: new THREE.Vector3(1,0,0) }))),
  new Promise(res => loader.load('/models/cube.obj', obj => res({ obj, scale: 0.3, pos: new THREE.Vector3(-6,0,0) })))
]).then(loadedMeshes => {
  loadedMeshes.forEach(({ obj, scale, pos }) => {
    obj.traverse(child => {
      if (child.isMesh) {
...
function finalSetUp(){
  ...
  // Build DataTexture for the shader
  const triangleSets = meshes.map(m => extractTrianglesFromMeshWorld(m));
  const { out1: triV0, out2: triV1, out3: triV2 } = toThreeLists(triangleSets);
  const { texture, width, height, totalTris } = makeTriangleTexture(triV0, triV1, triV2);
